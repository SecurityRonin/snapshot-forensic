# Contributing to snapshot-forensic

Thanks for your interest in improving `snapshot-forensic`. These crates parse
untrusted snapshot and backup container files acquired from potentially
...
> **Status:** early-stage scaffold. The format research in
> `docs/FORMAT_REFERENCE.md` is complete; the parser is under construction. The
> disciplines below apply to every line of parser code from the first commit.
...
snapshot/backup artifacts and an independent oracle (the tool that already
decodes the format correctly), not only synthetic fixtures.
...
- **Coverage** — 100% line coverage is enforced; no source line may be left
  ...
  happy path.
- **Fuzzing** — once the parser lands, every parsed structure gains a
  `cargo-fuzz` target whose invariant is "must not panic"; changes to a parser
  must keep its target green.
